...
//     try {
//       const res = await axios.get("/staff/assigned-complaints/");
...
//     try {
//       await axios.patch("/staff/update-assignment-status/", {
...
//       <h3 className="mb-4">👷 My Assigned Complaints</h3>
...
//     try {
//       const res = await axios.post("/staff/return-complaint/", {
//         assignment_id: assignmentId,
//         remarks: remark,
//       });
...
//           {assignments.map((a) => {
//             const isClosed =
//               a.status === "Resolved" || a.status === "Rejected" || a.status === "Returned";
...
//                       <strong>Complaint #{a.complaint_id}</strong>
//                       <br />
//                       <small>Staff ID: {a.staff_id || "N/A"}</small>
//                     </div>
...
//                     {isClosed && (
//                       <div className="alert alert-secondary mt-2 py-1 small">
...
//   const fetchAssignedComplaints = async () => {
//     try {
//       const res = await axios.get("/accounts/staff/assigned-complaints/");
//       setAssignments(res.data);
//     } catch (error) {
//       alert(error?.response?.data?.error || "Failed to load complaints");
//     } finally {
...
//     setUpdatingId(assignmentId);
//     try {
//       await axios.patch("/accounts/staff/update-assignment-status/", {
//         assignment_id: assignmentId,
//         status,
//       });
...
//       setAssignments((prev) =>
//         prev.map((a) =>
//           a.assignment_id === assignmentId ? { ...a, assignment_status: status } : a
//         )
//       );
...
//     try {
//       const res = await axios.post("/accounts/staff/return-complaint/", {
//         assignment_id: assignmentId,
//         remarks: remark,
//       });
...
//         prev.map((a) =>
//           a.assignment_id === assignmentId
//             ? { ...a, assignment_status: res.data.status }
//             : a
//         )
//       );
...
//                   <div className="card-header d-flex justify-content-between">
//                     <div>
//                       <strong>Complaint #{a.complaint.id}</strong>
//                       <br />
//                       <small>Staff ID: {a.staff_id || "N/A"}</small>
//                     </div>
...
//                     <p><strong>Priority:</strong> {a.complaint.priority}</p>
//                     <p><strong>Location:</strong> {a.complaint.location}</p>
//                     <p className="text-muted">{a.complaint.description}</p>

//                     {a.complaint.attachment && (
//                       <a
//                         href={a.complaint.attachment}
//                         target="_blank"
//                         rel="noreferrer"
//                         className="btn btn-outline-primary btn-sm w-100 mb-2"
//                       >
...
//                       className="form-select form-select-sm"
//                       disabled={isClosed}
//                       value={pendingStatus[a.assignment_id] ?? a.assignment_status}
//                       onChange={(e) =>
//                         setPendingStatus({
//                           ...pendingStatus,
//                           [a.assignment_id]: e.target.value,
...
//                         isClosed ||
//                         updatingId === a.assignment_id ||
//                         pendingStatus[a.assignment_id] === a.assignment_status
//                       }
//                       onClick={() => updateStatus(a.assignment_id)}
//                     >
//                       {updatingId === a.assignment_id
...
//       // ✅ FIX 1: Access the array inside the response object
...
//       // ✅ FIX 2: Use "status" not "assignment_status"
//       setAssignments((prev) =>
//         prev.map((a) =>
...
//       // ✅ FIX 3: Use "status" not "assignment_status"
//       setAssignments((prev) =>
//         prev.map((a) =>
//           a.assignment_id === assignmentId
//             ? { ...a, status: res.data.status }
...
//         <div className="row g-4">
//           {assignments.map((a) => {
//             // ✅ FIX 4: Use "status" not "assignment_status"
//             const isClosed =
//               a.status === "Resolved" || 
//               a.status === "Rejected" || 
//               a.status === "Returned";
...
//                   <div className="card-header d-flex justify-content-between">
//                     <div>
//                       <strong>Complaint #{a.complaint?.id || "N/A"}</strong>
//                       <br />
//                       {/* ✅ FIX 5: Remove or adjust staff_id display since backend doesn't send it */}
//                       <small>Assigned ID: {a.assignment_id || "N/A"}</small>
//                     </div>
//                     {/* ✅ FIX 6: Use "status" not "assignment_status" */}
//                     {getStatusBadge(a.status)}
//                   </div>

//                   <div className="card-body">
//                     <p><strong>Category:</strong> {a.complaint?.category || "N/A"}</p>
//                     <p><strong>Priority:</strong> {a.complaint?.priority || "N/A"}</p>
//                     <p><strong>Location:</strong> {a.complaint?.location || "N/A"}</p>
//                     <p className="text-muted">{a.complaint?.description || "No description"}</p>
...
//                       className="form-select form-select-sm"
//                       disabled={isClosed}
//                       // ✅ FIX 7: Use "status" not "assignment_status"
//                       value={pendingStatus[a.assignment_id] ?? a.status}
//                       onChange={(e) =>
//                         setPendingStatus({
//                           ...pendingStatus,
...
//       return `${departments[0]}, ${departments[1]} +${departments.length - 2}`;
//     }
//     return "N/A";
//   };
